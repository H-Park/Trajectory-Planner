...
float** smoothPath(int numWayPoints, int dimWayPoint, float** wayPoints, float totalTime, float timeStep)
{
    ...
    for (int i = 0; i < 3; i++) {
        if (i == 0) {
            ...
            //tmp container for unsmoothed exented path. Yeah C...
            float** tmp1  = inject(numPoints, dimWayPoint, path, injections[i]);

            //update the number of points in our path before proceeding
            numPoints = numPoints + ((injections[i]) * (numPoints- 1));

            //Another tmp container for smoothed exented path. Yeah C...
            float** tmp2 = smoother(numPoints, dimWayPoint, tmp1, pathAlpha, pathBeta, pathTolerance);
            ...
            //re-allocate to the size of [numPonts][dimWayPoint]
            path = malloc(numPoints * sizeof(float*));
            for(int i = 0; i < numPoints; i++) {
                path[i] = malloc(dimWayPoint* sizeof(float));
            }
        ...
        //update num_points now that path has numPoints
        old_num_points = numPoints;
    }
